...
shinyServer(function(input, output) {
    ...
    table_layout <- reactive({
        ...
        sketch <- htmltools::withTags(table( ## use this to keep tables not too long
            class = 'display',
            thead(
                tr(
    ...
    output$table <- DT::renderDataTable({
        ...
        selected_data() %>%
            group_by(.data[[input$groupvar]]) %>% ## gets around issue of shiny working with strings but shiny not wanting strings
            summarize(
                cases = n(),
    ...
    output$report <- downloadHandler( ## can use to download anything (picture/plot/data/etc.)
        # Set up report name and file type using user input
        filename = function() {
            paste('my-penguin-report', sep = '.', switch(
                ## allow for option of different types (html/pdf/docx)
                input$format, PDF = 'pdf', HTML = 'html', Word = 'docx'
            )) 
        },

        # Define content of the report
        ## linking filename to content
        content = function(file) { 
            ## as the report is being created it shows progress bar
            shiny::withProgress(
                message = paste0("Creating your report!"),
                value = 0,
                {
                    ## to create use `officedown` report (all within the if else statement below)
                    if(input$format != "Word") {
                        ...
                        ## use temporary directory if unable to write to server directory
                        owd <- setwd(tempdir()) 
                        on.exit(setwd(owd))
                        file.copy(src, 'report.Rmd', overwrite = TRUE)
                        file.copy(src2, 'penguins.png', overwrite = TRUE)

                        ## update progress bar after each thing is done
                ...
                }) # close out of progress bar ##remind yourself what this is closing out with a comment!
        }
    ...
    content = function(file) {
        shiny::withProgress(
            ...
            value = 0, {
                ...
                ## `fp_text_lite()` doesn't have defaults `fp_text()` has defaults and will override anyithing in your template
